...
Browser (SPA)
  └── Cloudflare Pages (static Vite build)
        ...
        │     ├── /api/auth           (Web Crypto PBKDF2 auth, login, instructor register, invite accept)
        │     ├── /api/admin          (Superadmin instructor approvals, user directory, invitation sender)
        │     ├── /api/courses        (Catalog, instructor studio, section/lesson CRUD, YouTube embeds)
        │     ├── /api/progress       (Real-time lesson checklist, dashboard progress, video watch logs)
        │     ├── /api/seed           (Local dev database initializer for demo accounts)
